...
namespace DesktopApp.Database
{
    public class Engine : IDisposable
    {
        ...
        {
            _server = Environment.GetEnvironmentVariable("SMILE_DB_SERVER") ?? "localhost";
            _port = Environment.GetEnvironmentVariable("SMILE_DB_PORT") ?? "3306";
            _database = Environment.GetEnvironmentVariable("SMILE_DB_NAME") ?? "smile_sunshine";
            _uid = Environment.GetEnvironmentVariable("SMILE_DB_USER");
            _password = Environment.GetEnvironmentVariable("SMILE_DB_PASSWORD");

            if (string.IsNullOrWhiteSpace(_uid) || string.IsNullOrEmpty(_password))
            {
                throw new InvalidOperationException(
                    "Database credentials are not configured. Set SMILE_DB_USER and SMILE_DB_PASSWORD before starting the application.");
            }
